...
export const cachePath = fm.joinPath(fm.documentsDirectory(), CACHE_FOLDER);
const device_id = `${Device.model()}_${Device.name()}`.replace(/[^a-zA-Z0-9\-\_]/, '').toLowerCase()

export const generateSlices = async function ({ caller = 'none' }) {
  const opts = { caller }
...
export const applyTint = function (widget, tint, alpha) {
  tint = tint || '#ffffff'
  alpha = alpha || 0.2
...
export const getSlice = async function (name) {
  ...
  if (!fm.fileExists(imgPath)) {
    ...
    var setupCompleted = await generateSlices({ caller: 'getSliceForWidget' })
    if (!setupCompleted) {
      return null
    }
  }
...
export const getPathForSlice = async function (slice_name) {
  let appearance = (await isUsingDarkAppearance())
    ? 'dark'
    : 'light'
  ...
  if (!fileExists) {
    fileExists = await generateSlices({ caller: 'self' })
  }
...
//------------------------------------------------
export const getSliceForWidget = async function (
  instance_name,
  reset = false) {
  ...
  if (!position) {
    ...
    if (!fm.fileExists(testImage)) {
      ...
      readyToChoose = await generateSlices({ caller: 'self' })
    } else {
      readyToChoose = true
    }
    ...
    // now set the 
    if (readyToChoose) {
      var backgrounChosen = await chooseBackgroundSlice(instance_name)
    }
...
export const chooseBackgroundSlice = async function (name) {
...
//------------------------------------------------
export const resetConfig = async function () {
  await saveConfig({})
